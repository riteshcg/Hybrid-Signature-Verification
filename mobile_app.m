...
k = dir('data') ;  % loading  files in the directory data 
 ...
 last=50;
  label_im=1;
  dataDir = fullfile('D:\FINA_code_demo_ansp\data set');
...
%    
  ...
  testIndices = vec2ind(testY1);
   ...
   pause(0.002)
     ...
     prog=0;
while (prog==0)
        data_new=exist('D:\ImageDetails\Image\test.png');
pause(1)
...
%  
queryImage = imread('D:\ImageDetails\Image\test.png');%'NF4.png');
data_new=0;
figure
imshow(queryImage)
...
%% CHECK QUERY 
best= dir(strcat('D:\FINA_code_demo_ansp\GENUINE\',output));
 %%
ctn=1;
%      % for original image
for i=3:length(best)
...
ctn=ctn+1;
 ...
 if percentage_evaluation >60
    str=' PERSON  CAN BE AUTHENTICATED';
else 
    str=' PERSON CANNOT BE AUTHENTICATED' ;
end 
   delete('D:\ImageDetails\Image\test.png');
 fileID = fopen('D:\FileInfo\Hi.txt','w');
  fprintf(fileID,'Total matched percentage is      %3.2f \n',percentage_evaluation); 
  fprintf(fileID,'\n'); 
 fprintf(fileID,'%c %c\n',str);
fclose(fileID);
prog=0;
